 ...
 *   (create, read, update, delete) для полученных с сервера данных:
 *   <pre>
 ...
 *   в зависимости от `isArray`). Когда данные будут получены с сервера, существующая ссылка заполняется 
 *   фактическими значениями. Это полезный трюк, т.к. обычно ресурсы назначаются для моделей, которые отображаются 
 *   в представлении. Когда ресурс пустой, он не отображается в представлении, но как только приходят данные, 
 *   представление перерисовывается, чтобы отобразить новые данные. Это означает, что в большинстве случаев не нужно 
 *   писать колбэк для методов действий.
 * 
 *   Методы действий на объекте класса или на экземпляре объекта могут быть вызваны со следующими параметрами:
 *
 *   - HTTP GET «класс» действия: `ресурс.действие([parameters], [success], [error])`
 *   - не GET «класс» действия: `ресурс.действие([parameters], postData, [success], [error])`
 *   - не GET экземпляр действия:  `экземпляр.$действие([parameters], [success], [error])`
 *
 *
 ...
 * <pre>
     ...
     var cards = CreditCard.query(function() {
       ...
       card.name = "J. Smith";
       // не GET методы применяются в экземплярах
       card.$save();
 ...
 * действия.
 * 
 * Выполнение этих методов вызывает `$http` с шаблоном URL-шаблоном с полученными методом `method`, параметрами `params`
 * и заголовками `headers`. Когда данные возвращены сервером, создается объект с определенным для ресурса 
 * типом, который содержит все не GET методы, доступные через методы с префиксом `$` . Это позволяет легко 
